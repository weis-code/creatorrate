...
export default function ResetPasswordPage() {
  ...
  const [state, setState] = useState<'idle' | 'loading' | 'done' | 'expired' | 'error'>('idle')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      // Sessionen er ikke gyldig — linket er udløbet
      if (error.message.toLowerCase().includes('session') || error.status === 401 || error.status === 403) {
        setState('expired')
      } else {
        setState('error')
      }
          ...
          <div className="p-8">
            {state === 'done' ? (
              ...
              <div className="text-center py-4">
                ...
                <p className="text-gray-700 font-medium mb-4">Dit link er udløbet eller ugyldigt.</p>
                <Link
                  href="/forgot-password"
                  className="inline-flex items-center justify-center bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:from-indigo-700 hover:to-purple-700 transition-all shadow-lg shadow-indigo-200"
                >
                  Anmod om nyt link →
                </Link>
              </div>
