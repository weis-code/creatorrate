...
export default function ShareProfileButton({ url }: { url: string }) {
  ...
  return (
    <button
      onClick={handleShare}
      className="flex items-center gap-2 bg-white/15 hover:bg-white/25 text-white border border-white/20 px-4 py-2 rounded-xl text-sm font-medium transition-colors"
    >
      {copied ? (
        <>
